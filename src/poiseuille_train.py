...
# =============================================================================
# 2. NEURAL NETWORK MODEL CONFIGURATION (Correct 3-Variable Model)
# =============================================================================
...
# =============================================================================
# 3. COLLOCATION POINT SAMPLING STRATEGY (Correct 3-Variable Version)
# =============================================================================
def generate_training_data():
    ...
    # Define IC targets (u, p, alpha only)
    ...
    # Return 3-variable target tuples
    return {
        'ic':(t_ic,x_ic,y_ic),
        'ic_targets':(u_ic,p_ic,alpha_ic),
        'ns':(t_ns,x_ns,y_ns),
        'ns_targets':(u_ns,p_ns),
        'ew':(t_ew,x_ew_west,x_ew_east,y_ew),
        'ew_targets':(p_ew_west_true,p_ew_east_true),
        'f':(t_f,x_f,y_f)
    }

# =============================================================================
# 4. LOSS FUNCTION IMPLEMENTATION (Correct 3-Variable Version)
# =============================================================================
...
# =============================================================================
# 5. TRAINING PROCEDURE (Correct 3-Variable Version)
# =============================================================================
...
def evaluate_model_mse_mae(model):
    N_eval = 100
    ...
    MAE = np.mean(np.abs(u_pred_norm - u_exact_norm))

    return mean_squared_error(u_exact_norm, u_pred_norm), MAE

def create_batches(data_dict, num_batches=10):
    """
    Shuffles and splits all training data into batches.
    """
    t_ic, x_ic, y_ic = data_dict['ic']
    u_ic, p_ic, alpha_ic = data_dict['ic_targets']
    
    t_ns, x_ns, y_ns = data_dict['ns']
    u_ns, p_ns = data_dict['ns_targets']

    t_ew, x_ew_w, x_ew_e, y_ew = data_dict['ew']
    p_ew_w, p_ew_e = data_dict['ew_targets']
    ...
    for i in range(num_batches):
        batches.append({
            'ic': (t_ic_b[i], x_ic_b[i], y_ic_b[i]), 
            'ic_targets': (u_ic_b[i], p_ic_b[i], alpha_ic_b[i]),
            'ns': (t_ns_b[i], x_ns_b[i], y_ns_b[i]), 
            'ns_targets': (u_ns_b[i], p_ns_b[i]),
            'ew': (t_ew_b[i], x_ew_w_b[i], x_ew_e_b[i], y_ew_b[i]), 
            'ew_targets': (p_ew_w_b[i], p_ew_e_b[i]),
...
if __name__ == "__main__":
    ...
    best_mse = float('inf')
    best_model_basename = "best_pinn_model"
    ...
    for stage in training_stages:
        ...
        for epoch in range(epochs):
            ...
            if total_epochs % NUM_EPOCHS_BEFORE_CHECKPOINT == 0:
                current_mse, current_mae = evaluate_model_mse_mae(pinn_model)
                if USE_ADAPTIVE_ACTIVATION:
                    layer_means = [tf.reduce_mean(a).numpy() for a in pinn_model.a_s]
                    total_mean = sum(layer_means) / len(layer_means)
                    print(f"Epoch: {total_epochs:05d}, Loss: {total_loss:.4e}, IC: {loss_ic:.4e}, BC: {loss_bc:.4e}, PDE: {loss_f:.4e}, Mean A: {total_mean:.4f}, MSE: {current_mse:.4e}, MAE: {current_mae:.4e}")
                else:
                    print(f"Epoch: {total_epochs:05d}, Loss: {total_loss:.4e}, IC: {loss_ic:.4e}, BC: {loss_bc:.4e}, PDE: {loss_f:.4e}, MSE: {current_mse:.4e}, MAE: {current_mae:.4e}")
                ...
                if current_mse < best_mse:
                    ...
                    best_weights_file_path = str(run_save_dir / best_weights_filename)
